...
public:
    NavigationNode(std::shared_ptr<Navigator> navigator) : Node("navigation_node"), navigator_(navigator){;
        ...
        setInitialPose(2.12, -21.3, 1.57); //publish initial pose
...
private:
    ...
    // std::vector<geometry_msgs::msg::Pose> final_human_locations_;
    // bool humans_found_ = false;
    ...
    void callback(){
        ...
        if(current_waypoint_phase_ ==0){
            // check if next goal needs to be sent
            if(!sent_goal_){
                ...
                //navigation complete, switch to the spin phase
                sent_goal_ = false;
        ...
        if(current_waypoint_phase_ == 1){
            // check if spin goal needs to be sent
            if(!sent_goal_){
    ...
    void amclCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg){
        //store latest amcl pose for main control loop
        last_amcl_pose_ = msg;
    }
    // void humansCallback(const geometry_msgs::msg::PoseArray::SharedPtr msg) {
    //     if (!msg->poses.empty() && !humans_found_) {
    //         // Stop exploration and store final result
    //         RCLCPP_INFO(this->get_logger(), "!!! Humans have been located! Stopping exploration.");
    //         humans_found_ = true;
    //         final_human_locations_ = msg->poses;

    //         //log the final discovered locations
    //         for (const auto& pose : final_human_locations_) {
    //             RCLCPP_INFO(this->get_logger(), "Human located at (X: %.2f, Y: %.2f)", 
    //                         pose.position.x, pose.position.y);
    //         }

    //         //ttop the timer so no more navigation commands are sent
    //         timer_->cancel(); 
    //     }
    // }
};
